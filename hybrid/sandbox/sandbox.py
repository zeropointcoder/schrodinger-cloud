...
class PhaseEstimation:
    ...
    def _apply_controlled_unitaries(self, qubits):
        ...
        for i in range(self.num_ancilla_qubits):
            ...
            # In this case, we're using controlled-Z (as an example for U).
            for j in range(i + 1):
    ...
    def _inverse_qft(self, qubits):
        ...
        # Apply the inverse QFT: Reverse of QFT steps
        for i in range(self.num_ancilla_qubits):
...
pe = PhaseEstimation(num_ancilla_qubits=2, unitary=cirq.Z)  # Using controlled-Z as an example unitary
